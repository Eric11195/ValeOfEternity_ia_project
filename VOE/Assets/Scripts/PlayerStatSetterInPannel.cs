...
namespace voe {
    public class PlayerStatSetterInPannel : MonoBehaviour
    {
        ...
        {
            img.sprite = Resources.Load<Sprite>("cup"+st.ranking.ToString());
            Assert.IsTrue(img.sprite != null);
            ...
            list[8].text = st.points_per_infinite_effect.ToString();
            list[9].text = st.wasted_stones.ToString();
            list[10].text = st.wasted_stones_value.ToString();
